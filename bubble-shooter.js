...
window.onload = function() {
	...
	// Load the bubble pop sound
	var popSound = new Audio('bubble_pop_sound.mp3');

	// Function to play the bubble pop sound
    ...
    function renderPlayer() {
		...
		// Draw the angle
		context.lineWidth = 2;
		context.strokeStyle = "#00008B"; // changed to red
		context.beginPath();
		var endX = centerx + 1.5 * level.tilewidth * Math.cos(degToRad(player.angle));
		var endY = centery - 1.5 * level.tileheight * Math.sin(degToRad(player.angle));
		context.moveTo(centerx, centery);
		context.lineTo(endX, endY);

		// Draw the head of the arrow
		var headlen = 10;   // length of head in pixels
		var angle = Math.atan2(endY - centery, endX - centerx); // Switched around endY and centery
    ...
    // Threshold for bubble sliding
        var threshold = 10; // Change this value as you see fit
